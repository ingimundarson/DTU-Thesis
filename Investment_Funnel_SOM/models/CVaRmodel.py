...
# ----------------------------------------------------------------------
def cvar_model(test_ret, scenarios, targets, budget, cvar_alpha, trans_cost, max_weight, n_rebalancing = 4):
    """
    ...
    for w in test_ret.index[0:n_rebalancing]:
    ...
    for p in range(1, p_points):
        ...
        p_alloc, cvar_val, port_val = rebalancing_model(mu=expected_returns,
                                                        ...
                                                        cvar_targets=targets[p] * portfolio_value_w,
                                                        cvar_alpha=cvar_alpha,
        ...
        portfolio_value_w = port_val
        # COMPUTE PORTFOLIO VALUE
        for w in test_ret.index[(p * n_rebalancing): (n_rebalancing + p * n_rebalancing)]:
